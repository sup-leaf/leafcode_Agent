...
# BrowserSession 只管理 Playwright 生命周期，不做模型决策。
...
class BrowserTools:
    ...
    def observe(self) -> PageSnapshot:
        ...
        elements = tuple(
            InteractiveElement(
                ...
                item["text"],
                f'[data-leafcode-element="{item["id"]}"]',
                "type" if item["tag"] in ("input", "textarea") else "click",
                {
                    key: str(item.get(key, ""))
                    for key in ("href", "type", "name", "placeholder", "value")
                    if item.get(key)
                },
            )
        ...
        content = json.dumps(
            {
                ...
                "scrollY": raw["scrollY"],
                "elements": [
                    (e.id, e.text, e.attributes.get("value", "")) for e in elements
                ],
            },
            ensure_ascii=False,
        )
        ...
        self.current_snapshot, self._elements = snapshot, {
    ...
    def format_snapshot(snapshot: PageSnapshot) -> str:
        ...
        lines.extend(
            f"  [{e.id}] {e.role} {e.text[:60]} {e.attributes.get('href', '')[:60]}"
    ...
    def _finish(
        ...
        *,
        ...
        error = error or (
            "操作后页面没有可验证的变化" if require_change and not changed else None
        )
        return ToolResult(
            success,
            message if success else f"{message}；{error}",
            before,
            after,
            changed,
            error,
    ...
    def click(self, element_id: str) -> ToolResult:
        before = self._begin()
        try:
            self.page.locator(self._element(element_id, "click").selector).click(
                timeout=10_000
            )
    ...
    def type(self, element_id: str, text: str) -> ToolResult:
        before = self._begin()
        try:
            self.page.locator(self._element(element_id, "type").selector).fill(
                text, timeout=10_000
            )
